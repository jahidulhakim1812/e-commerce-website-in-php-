...
// Fetch only orders with pending shipping
$orders = $pdo->query("
...
<head>
<meta charset="UTF-8">
<title>Pending Shipping Orders - Admin Panel</title>
                ...
                <?php if(!empty($orders)): ?>
                    <?php foreach ($orders as $order): ?>
                        <tr>
                            ...
                            <td data-label="Order Status" class="<?= $order['status'] ? 'status-complete' : 'status-pending' ?>">
                                <?= $order['status'] ? 'Completed' : 'Pending' ?>
                            </td>
                            <td data-label="Shipping Status" class="status-pending">Pending</td>
                            <td data-label="Date"><?= date('M d, Y H:i', strtotime($order['created_at'])) ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="8" class="empty-msg">No pending shipping orders found.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
